...
public abstract class SharedEntityManagerCreator {
	...
	private static final Map<Class<?>, Class<?>[]> QUERY_INTERFACES = new ConcurrentReferenceHashMap<>();
	 ...
	 * @param emf the EntityManagerFactory to delegate to.
	 * @param properties the properties to be passed into the {@code createEntityManager} call (may be {@code null})
	 * @param synchronizedWithTransaction whether to automatically join ongoing transactions (according to the JPA 2.1
	 *          SynchronizationType rules)
	 * @return a shareable transaction EntityManager proxy
	 * @since 4.0
	 */
	public static EntityManager createSharedEntityManager(EntityManagerFactory emf, @Nullable Map<?, ?> properties,
	 ...
	 * @param properties the properties to be passed into the {@code createEntityManager} call (may be {@code null})
	 * @param entityManagerInterfaces the interfaces to be implemented by the EntityManager. Allows the addition or
	 *          specification of proprietary interfaces.
	...
	public static EntityManager createSharedEntityManager(EntityManagerFactory emf, @Nullable Map<?, ?> properties,
		...
		return (EntityManager) Proxy.newProxyInstance(cl != null ? cl : SharedEntityManagerCreator.class.getClassLoader(),
				ifcs, new SharedEntityManagerInvocationHandler(emf, properties, synchronizedWithTransaction));
	}

	/**
	 * Invocation handler that delegates all calls to the current transactional EntityManager, if any; else, it will fall
	 * back to a newly created EntityManager per operation.
	 */
	...
	private static class SharedEntityManagerInvocationHandler implements InvocationHandler, Serializable {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			if (method.getName().equals("getTargetEntityManager")) {
				...
				if (target == null
						|| !TransactionSynchronizationManager.isActualTransactionActive() && !target.getTransaction().isActive()) {
			...
			if (target == null) {
				...
				target = !CollectionUtils.isEmpty(this.properties) ? this.targetFactory.createEntityManager(this.properties)
			...
			try {
				...
				if (result instanceof Query) {
					...
					if (isNewEm) {
						Class<?>[] ifcs = QUERY_INTERFACES.computeIfAbsent(query.getClass(),
								it -> ClassUtils.getAllInterfacesForClass(it, this.proxyClassLoader));
	 ...
	 * <p>
	 * Includes deferred output parameter access for JPA 2.1 StoredProcedureQuery, retrieving the corresponding values for
	 * all registered parameters on query termination and returning the locally cached values for subsequent access.
	 */
